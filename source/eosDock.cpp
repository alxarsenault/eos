...
eos::Dock::Dock(const ax::Rect& rect)
	...
	, _anim_up(true)
	, _up_rect(rect)
	, _drop_rect(ax::Rect(rect.position.x, rect.position.y + rect.size.y - 10,
		rect.size.x, 10))
{
	ax::App& app = ax::App::GetInstance();
	_timer_up = new ax::Event::Timer(app.GetEventManager(),
			GetOnAnimationTimerUp());
	
	_timer_down = new ax::Event::Timer(app.GetEventManager(),
	...
	for(auto& n : apps_icon_info)
	{
		...
		ax::Window::Ptr icon = win->node.Add(std::shared_ptr<eos::DockIcon>(
				new eos::DockIcon(ax::Rect(icon_pos, icon_size),
						n.first, n.second)));
		...
		const ax::Color& c1, const ax::Color& c2)
{
	...
	ax::FloatRect frect(rect.position.x, rect.position.y,
			rect.size.x, rect.size.y);
	
	// Middle.
	DrawRectangleColorFade(gc, ax::Rect(frect.position.x + r - 1,
			frect.position.y - 1, frect.size.x - 2.0 * r + 1,
			frect.size.y + 1), c1, c2);
	
	int size_rect_height = frect.size.y - 1.0 * r + 1;
	
		// Left.
		ax::FloatRect lrect(frect.position.x - 1,
				frect.position.y + r - 1, r, size_rect_height);
	...
	// Top left.
	DrawQuarterCircle(gc, ax::FloatPoint(frect.position.x + r - 1 ,
			frect.position.y + r - 1), r, M_PI, lc1, c1);
	
	// Top right.
	DrawQuarterCircle(gc, ax::FloatPoint(frect.position.x + frect.size.x - r,
			frect.position.y + r - 1), r, 3.0 * M_PI * 0.5, lc1, c1);
}
...
void eos::Dock::OnPaint(ax::GC gc)
{
	...
	if(_isDrop) {
		...
		if(_anim_active) {
			...
			DrawRoundedRectangle(gc, dock_rect, 25,
					c_contour, c_dock);

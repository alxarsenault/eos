...
//_system(system)
{
	...
	_last_icon_index = children.size();
	...
	//	eos::AppViewer* appViewer = new eos::AppViewer(this, ax::Rect(0, 24,
	// 300, rect.size.y));
	appViewer->Hide();
...
void eos::Desktop::AddFrame(std::shared_ptr<ax::Window::Backbone> frame)
{
	...
	children.insert(children.begin() + _last_icon_index, child);
	_last_icon_index++;
...
//	[&](axWindowNode* node, ax::Rect main_rect, ax::Rect draw_rect, ax::Point
// delta_pos)
//	{
...
//
//		ax::Size size = ax::Size(win_rect.size.x / double(main_rect.size.x) *
// draw_rect.size.x,
//								 win_rect.size.y / double(main_rect.size.y) *
// draw_rect.size.y);
//

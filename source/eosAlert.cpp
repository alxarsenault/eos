	...
	, _time_font("resource/DejaVuSansMono.ttf")
{
	...
	win->event.OnPaint = ax::WBind<ax::GC>(this, &Alert::OnPaint);
//	_system = nullptr;
//	_time_font.SetFontSize(10);
//	_tracer_stream.open(
//		_system->GetTracer()->GetTraceFilePath(), std::fstream::in);
...
int DrawString(
	ax::GC& gc, ax::Font& font, const std::string text, const ax::Point& pos)
{
	int x = pos.x;
...
void eos::Alert::OnPaint(ax::GC gc)
{
	...
	for (int i = 0; i < _trace.size(); i++) {
		gc.SetColor(ax::Color(0.0f, 0.8f, 0.0f));
